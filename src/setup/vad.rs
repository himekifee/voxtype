...
use crate::vad::{
    get_onnx_vad_model_dirname, get_onnx_vad_model_filename, get_onnx_vad_model_url,
...
/// Download the ONNX Whisper encoder-decoder VAD model
pub fn download_onnx_model() -> anyhow::Result<()> {
    let model_dir = Config::models_dir().join(get_onnx_vad_model_dirname());
    let model_path = model_dir.join(get_onnx_vad_model_filename());

    if model_path.exists() {
        print_success(&format!(
            "ONNX VAD model already installed: {:?}",
            model_path
        ));
        print_info("To re-download, delete the directory and run this command again.");
        return Ok(());
    }
    ...
    println!("Downloading ONNX VAD model...");
    println!("URL: {}", url);
    ...
    let status = Command::new("curl")
        .args([
            ...
            "--progress-bar",
            "-o",
            model_path.to_str().unwrap_or("model.onnx"),
            url,
        ])
        .status();
...
pub fn show_status() {
    ...
    let filename = get_whisper_vad_model_filename();
    let model_path = models_dir.join(filename);
    let onnx_model_path = models_dir
        .join(get_onnx_vad_model_dirname())
        .join(get_onnx_vad_model_filename());
    ...
    if onnx_model_path.exists() {
        let size = std::fs::metadata(&onnx_model_path)
            .map(|m| m.len())
            .unwrap_or(0);
        print_success(&format!(
            "ONNX VAD model installed: {:?} ({:.1} MB)",
            onnx_model_path,
            size as f64 / 1_048_576.0
        ));
    } else {
        print_warning("ONNX VAD model not installed");
        print_info("Download with: voxtype setup vad --onnx");
    }
}
